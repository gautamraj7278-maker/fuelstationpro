...
-- total_deductible = ABS(variance) - ABS(exemptions) — the amount deducted from operator.
...
-- Migrate old columns if table was previously created with deduction_amount / net_payable
...
INSERT INTO operator_sales_settlements (sale_date, shift_name, operator_name, dispenser_name, daily_sales_entry_id, total_sales_amount, submitted_amount, variance, exemptions, total_deductible)
SELECT
  d.sale_date,
  d.shift_name,
  d.operator_name,
  ...
  d.variance,
  0 as exemptions,
  abs(d.variance) as total_deductible
